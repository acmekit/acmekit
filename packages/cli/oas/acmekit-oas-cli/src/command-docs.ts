...
const acmekitPluginAbsolutePath = path.resolve(
  basePath,
  "redocly/plugins/acmekit/index.js"
)
...
const createTmpConfig = async (
  ...
  config.plugins = (config.plugins ?? []).filter(
    (plugin) => plugin !== acmekitPluginRelativePath
  )
...
`
      ...
      Object.keys(recommendation).forEach((recKey) => {
        originalContent.decorators["acmekit/circular-patch"].schemas[recKey] = [
          ...(originalContent.decorators["acmekit/circular-patch"].schemas[
            recKey
          ] || []),
