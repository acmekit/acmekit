...
function buildLocalCommands(cli, isLocalProject) {
  ...
  function resolveLocalCommand(command) {
    if (!isLocalProject && command !== "new") {
      console.error(
        `The "${command}" command must be run inside a AcmeKit project. Make sure you are in the root directory of a AcmeKit project and try again.`
      )
      process.exit(1)
    }

    try {
      const cmdPath = resolveCwd.silent(`@acmekit/acmekit/commands/${command}`)!
...
function isLocalAcmeKitProject() {
  ...
  try {
    ...
    // Draft order plugin can't have @acmekit/acmekit as dependency,
    // so we also check for @acmekit/cli 
    inAcmeKitProject = !!(
      (dependencies &&
        (dependencies["@acmekit/acmekit"] || dependencies["@acmekit/cli"])) ||
      (devDependencies &&
        (devDependencies["@acmekit/acmekit"] ||
          devDependencies["@acmekit/cli"]))
    )
  } catch (err) {
    // ignore
  }
