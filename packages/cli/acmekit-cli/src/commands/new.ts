...
import { Pool } from "@acmekit/deps/pg"
import url from "url"
...
import { getNodeVersion, MIN_SUPPORTED_NODE_VERSION } from "@acmekit/utils"
...
const copy = async (starterPath, rootPath) => {
  ...
  if (starterPath === `.`) {
    throw new Error(
      ...
      something like "acmekit new my-acmekit-store ../local-acmekit-starter"`
    )
  }
...
export const newStarter = async (args) => {
  ...
  if (nodeVersion < MIN_SUPPORTED_NODE_VERSION) {
    reporter.error(
      `AcmeKit requires at least v20 of Node.js. You're using v${nodeVersion}. Please install at least v20 and try again: https://nodejs.org/en/download`
    )
    process.exit(1)
  }
  track("CLI_NEW")
