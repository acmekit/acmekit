 ...
 *
 * @throws {AcmeKitError} INVALID_DATA if any cart item is for a variant that has no inventory items.
 * @throws {AcmeKitError} INVALID_DATA if any cart item is for a variant with no stock locations in the input.sales_channel_id. An exception is made for variants with allow_backorder set to true.
 *
