import type { ConfirmVariantInventoryWorkflowInputDTO } from "@acmekit/framework/types"
import {
  WorkflowData,
  WorkflowResponse,
  createWorkflow,
...
import type { BigNumberInput } from "@acmekit/framework/types"
...
export interface ConfirmVariantInventoryWorkflowOutput {
  ...
  items: {
    ...
    /**
     * The number of units a single quantity is equivalent to. For example, if a customer orders one quantity of the variant, AcmeKit checks the availability of the quantity multiplied by the
     * value set for `required_quantity`. When the customer orders the quantity, AcmeKit reserves the ordered quantity multiplied by the value set for `required_quantity`.
     */
 ...
 * :::note
 *
 * Learn more about the links between the product variant and sales channels and inventory items in [this documentation](https://docs.acmekit.com/resources/commerce-modules/product/links-to-other-modules).
 *
 * :::
 *
 ...
 *
 * @example
 * You can retrieve a variant's required details using [Query](https://docs.acmekit.com/learn/fundamentals/module-links/query):
 *
 * ```ts workflow={false}
 ...
 * :::note
 *
 * In a workflow, use [useQueryGraphStep](https://docs.acmekit.com/resources/references/acmekit-workflows/steps/useQueryGraphStep) instead.
 *
 * :::
 *
