 ...
 * [Create Cart Store API Route](https://docs.acmekit.com/api/store#carts_postcarts).
 *
 * This workflow has a hook that allows you to perform custom actions on the created cart. You can see an example in [this guide](https://docs.acmekit.com/resources/commerce-modules/cart/extend#step-4-consume-cartcreated-workflow-hook).
 *
 ...
 *
 * ```ts
 * import { createCartWorkflow } from "@acmekit/acmekit/core-flows";
...
export const createCartWorkflow = createWorkflow(
  ...
  (input: WorkflowData<CreateCartWorkflowInput>) => {
    ...
    const cartInput = transform(
      ...
      (data) => {
        if (!data.region) {
          throw new AcmeKitError(AcmeKitError.Types.NOT_FOUND, "No regions found")
        }
