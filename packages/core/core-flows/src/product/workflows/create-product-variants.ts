...
export type CreateProductVariantsWorkflowInput = {
  ...
  product_variants: (ProductTypes.CreateProductVariantDTO & {
    ...
    inventory_items?: {
       ...
       * AcmeKit checks the availability of the quantity multiplied by the value set for `required_quantity`.
       * When the customer orders the quantity, AcmeKit reserves the ordered quantity multiplied by the value
...
/**
 * This workflow creates one or more product variants. It's used by the [Create Product Variant Admin API Route](https://docs.acmekit.com/api/admin#products_postproductsidvariants).
 *
