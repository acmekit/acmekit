...
import type { HttpTypes, IFileModuleService } from "@acmekit/framework/types"
import {
  CSVNormalizer,
  AcmeKitError,
  Modules,
...
import { AcmeKitErrorTypes } from "@acmekit/utils"
    ...
    >(async (resolve, reject) => {
      try {
        ...
        if (error instanceof CsvError) {
          return reject(
            new AcmeKitError(AcmeKitErrorTypes.INVALID_DATA, error.message)
          )
        }
        reject(error)
      }
