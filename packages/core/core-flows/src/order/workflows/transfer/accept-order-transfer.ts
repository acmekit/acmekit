...
import type { OrderPreviewDTO } from "@acmekit/framework/types"
import {
  ChangeActionType,
  AcmeKitError,
  OrderChangeStatus,
...
export const acceptOrderTransferValidationStep = createStep(
  ...
  async function ({
    ...
    if (!token.length || token !== transferCustomerAction?.details!.token) {
      throw new AcmeKitError(AcmeKitError.Types.NOT_ALLOWED, "Invalid token.")
    }
  }
)
 ...
 * [Accept Order Transfer Store API Route](https://docs.acmekit.com/api/store#orders_postordersidtransferaccept).
 *
