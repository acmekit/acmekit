...
import { MathBN, AcmeKitError } from "@acmekit/framework/utils"
import {
  WorkflowData,
  createStep,
  createWorkflow,
 ...
 * :::note
 *
 * You can retrieve a return details using [Query](https://docs.acmekit.com/learn/fundamentals/module-links/query),
...
export const cancelReturnValidateOrder = createStep(
  ...
  ({ orderReturn }: CancelReturnValidateOrderInput) => {
    ...
    const throwErrorIf = (
      ...
      if (arr?.some(pred)) {
        throw new AcmeKitError(AcmeKitError.Types.NOT_ALLOWED, message)
      }
    }
 ...
 * [Cancel Return Admin API Route](https://docs.acmekit.com/api/admin#returns_postreturnsidcancel).
 *
