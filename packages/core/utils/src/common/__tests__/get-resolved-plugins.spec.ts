...
describe("getResolvedPlugins | relative paths", () => {
  test("resolve configured plugins", async () => {
    await fs.createJson("node_modules/@acmekit/test-plugin/package.json", {
      name: "@acmekit/test-plugin",
      version: "1.0.0",
    })
    ...
    expect(plugins).toEqual(
      expect.arrayContaining([
        {
          id: "@acmekit/test-plugin",
          modules: [],
          name: "@acmekit/test-plugin",
          options: {},
          resolve: path.join(
            fs.basePath,
            "node_modules/@acmekit/test-plugin/.medusa/server/src"
          ),
          version: "1.0.0",
        },
        {
