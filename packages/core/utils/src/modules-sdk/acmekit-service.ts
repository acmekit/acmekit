...
import { createAcmeKitMikroOrmEventSubscriber } from "./create-acmekit-mikro-orm-event-subscriber"
...
import { isAcmeKitInternalService } from "./acmekit-internal-service"
import {
  BaseMethods,
...
/**
 * Symbol to mark a class as a AcmeKit service
 */
...
/**
 * Check if a value is a AcmeKit service
 * @param value
 */
export function isAcmeKitService(
  value: any
    ...
    constructor(container: Record<any, any>) {
      ...
      if (joinerConfig?.serviceName !== "index") {
        let globalSubscriber!: ReturnType<
          typeof createAcmeKitMikroOrmEventSubscriber
        >
        ...
        Object.keys(container)
          ...
          .forEach((key: string) => {
            globalSubscriber ??= createAcmeKitMikroOrmEventSubscriber(
              ["__acmekit_entities_subscriber__"],
              this
            )
            try {
              const service = container[key]
              if (isAcmeKitInternalService(service)) {
