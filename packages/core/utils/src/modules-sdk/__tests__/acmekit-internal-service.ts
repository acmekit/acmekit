import { AcmeKitInternalService } from "../acmekit-internal-service"
...
describe("Internal Module Service Factory", () => {
  ...
  const IAcmeKitInternalService = AcmeKitInternalService<any>(Model)

  describe("Internal Module Service Methods", () => {
    let instance

    beforeEach(() => {
      instance = new IAcmeKitInternalService(containerMock)
    })
    ...
    it("should throw an error on retrieve if composite key values are not defined", async () => {
      ...
      const compositeIAcmeKitInternalService =
        AcmeKitInternalService<any>(CompositeModel)

      const instance = new compositeIAcmeKitInternalService(containerMock)
