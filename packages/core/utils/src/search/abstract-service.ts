import { SearchTypes } from "@acmekit/types"
 ...
 *  from the `@acmekit/utils` package.
 *
 ...
 * import { AbstractSearchService } from "@acmekit/utils"
 *
 ...
 *
 * Although there are several helper methods in this class, the main methods used by the AcmeKit backend are `addDocuments`, `deleteDocument`, and `search`.
  ...
  implements SearchTypes.ISearchService
{
  ...
  /**
   * This property is used to pinpoint the default search service defined in the AcmeKit core. For custom search services, the `isDefault` property must be `false`.
   */
  ...
  /**
   * You can use the `constructor` of your search service to access the different services in AcmeKit through dependency injection.
   *
   * You can also use the constructor to initialize your integration with the third-party provider. For example, if you use a client to connect to the third-party provider’s APIs,
   * you can initialize it in the constructor and use it in other methods in the service.
   *
   * Additionally, if you’re creating your search service as an external plugin to be installed on any AcmeKit backend and you want to access the options added for the plugin,
   ...
   *
   * @param {Record<string, unknown>} cradle - An container cradle that allows you to access other resources, such as services, in your AcmeKit backend.
   ...
   * @example
   * // ...
   * import { ProductService } from "@acmekit/acmekit"
   *
   ...
   * ```
   *
   * Another example of how the [MeiliSearch plugin](https://docs.acmekit.com/plugins/search/meilisearch) uses the
   * `options` parameter:
   *
   * ```ts
   ...
   *
   * When the AcmeKit backend loads, it triggers indexing for all products available in the AcmeKit backend, which uses this method to add or update documents.
   ...
   *
   * When a product is deleted in the AcmeKit backend, this method is used to delete the product from the search engine’s index.
   *
   ...
   *
   * In the AcmeKit backend, this method is used within the [Search Products API Route](https://docs.acmekit.com/api/store#products_postproductssearch)
   ...
   *
   * For example, in the Algolia plugin, a loader, which runs when the AcmeKit backend loads, is used to update the settings of indices based on the plugin options.
