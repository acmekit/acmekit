...
import { dynamicImport } from "@acmekit/utils"
...
const isHealthCheck = (req: AcmeKitRequest) => req.path === "/health"
...
export async function expressLoader({
   ...
   */
  function shouldSkipHttpLog(req: AcmeKitRequest, res: AcmeKitResponse) {
    return (
      isTest ||
      isHealthCheck(req) ||
