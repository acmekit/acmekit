 ...
 *   ".acmekit/server" directory. The admin source and integration-tests are
 *   skipped.
 *
...
export class Compiler {
  ...
  constructor(projectRoot: string, logger: Logger) {
    ...
    this.#adminOnlyDistFolder = path.join(this.#projectRoot, ".acmekit/admin")
    this.#pluginsDistFolder = path.join(this.#projectRoot, ".acmekit/server")
   ...
   * or uses the ".acmekit/server" folder
   */
  #computeDist(tsConfig: { options: { outDir?: string } }): string {
    const distFolder = tsConfig.options.outDir ?? ".acmekit/server"
  ...
  /**
   * Loads the acmekit config file and prints the error to
   * the console (in case of any errors). Otherwise, the
   * file path and the parsed config is returned
   */
  async #loadAcmeKitConfig() {
    const { configModule, configFilePath, error } =
      await getConfigFile<ConfigModule>(this.#projectRoot, "acmekit-config")
    if (error) {
      this.#logger.error(`Failed to load acmekit-config.(js|ts) file`)
  ...
  /**
   * Builds the frontend source code of a AcmeKit application
   * using the "@acmekit/admin-bundler" package.
   */
  async buildAppFrontend(
    ...
    /**
     * Step 1: Load the acmekit config file to read
     * admin options
     */
    const configFile = await this.#loadAcmeKitConfig()
    if (!configFile) {
      return false
    }
    ...
    if (configFile.configModule.admin.disable && !adminOnly) {
      this.#logger.info(
        "Skipping admin build, since its disabled inside the acmekit-config file"
      )
      return true
    }
    ...
    if (!configFile.configModule.admin.disable && adminOnly) {
      this.#logger.warn(
        `You are building using the flag --admin-only but the admin is enabled in your acmekit-config, If you intend to host the dashboard separately you should disable the admin in your acmekit config`
      )
    }
