...
export class Client {
  ...
  private DEFAULT_JWT_STORAGE_KEY = "acmekit_auth_token"
  private token = ""
  ...
  setLocale(locale: string) {
    if (!window) {
      this.logger.warn(
        "setLocale is not available in the server environment. Please set the locale directly through the 'x-acmekit-locale' header."
      )
      return
    }
  ...
  protected initClient(): ClientFetch {
    ...
    return async (input: FetchInput, init?: FetchArgs) => {
      ...
      if (this.locale) {
        headers.set("x-acmekit-locale", this.locale)
      }

      const customHeaders = {
        "x-acmekit-locale": this.locale,
