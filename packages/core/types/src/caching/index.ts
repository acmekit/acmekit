...
type Providers = (string | { 
  /**
   * The ID of the provider to use, as set in `acmekit-config.ts`.
   */
  id: string;
  /**
...
export interface ICachingModuleService extends IModuleService {
  ...
  get({
     ...
     * If not provided, the [default provider](https://docs.acmekit.com/infrastructure-modules/caching/providers#default-caching-module-provider) is used.
     */
    providers?: string[]
  }): Promise<any | null>
   ...
   * [default Caching Module Provider](https://docs.acmekit.com/infrastructure-modules/caching/providers#default-caching-module-provider).
   *
   ...
   * :::note
   * 
   * Tags should follow [conventions](https://docs.acmekit.com/infrastructure-modules/caching/concepts#caching-tags-convention) to ensure they're automatically invalidated.
   * 
   * :::
   * 
   * ```ts
 ...
 *
 * If you're creating a client or establishing a connection with a third-party service, do it in a [Loader](https://docs.acmekit.com/learn/fundamentals/modules/loaders)
 * and store it in the Module's container. Then, you can access it in your service using the container.
 * 
 * :::note[Loader Example]
 * 
 * [Initialize MongoDB client in loader and access it in service](https://docs.acmekit.com/learn/fundamentals/modules/loaders#example-register-custom-mongodb-connection).
 * 
 * :::
 *
 * #### Example
 ...
 * will be stored as `lp_{identifier}_{id}`, where `id` is the ID you set in your `acmekit-config.ts` file.
 * 
 * For example:
 * 
 * ```ts
...
export interface ICachingStrategy {
   ...
   *
   * @param container AcmeKitContainer
