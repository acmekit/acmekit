...
export const dbTestUtilFactory = (): any => ({
  ...
  shutdown: async function (dbName: string) {
    try {
      ...
      // DROP DATABASE WITH (FORCE) atomically terminates all remaining
      // connections (including any that reconnected after pool.destroy())
      // and drops the database in a single lock-free operation.
      // This avoids the race between pg_terminate_backend and DROP DATABASE
      // where pool auto-reconnect can slip in new connections.
      // Requires PostgreSQL 13+ (Medusa v2 minimum supported version).
