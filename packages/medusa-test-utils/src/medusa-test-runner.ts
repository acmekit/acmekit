...
class AcmeKitTestRunner {
  ...
  public async cleanup(): Promise<void> {
    try {
      ...
      await this.shutdown()
      await execOrTimeout(this.dbUtils.shutdown(this.dbName), 30000)
      await clearInstances()
      ...
      if (this.globalContainer?.dispose) {
        await execOrTimeout(this.globalContainer.dispose(), 5000).catch(
          (error) => {
            logger.error(
              "globalContainer.dispose timed out or failed (proceeding):",
              error
            )
          }
        )
