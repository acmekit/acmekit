...
export default async function adminLoader({
  ...
  if (NOT_ALLOWED_PATHS.includes(adminOptions.path)) {
    logger.error(
      `The 'admin.path' in 'acmekit-config.js' is set to a value that is not allowed. This can prevent your server from working correctly. Please set 'admin.path' to a value that is not one of the following: ${NOT_ALLOWED_PATHS.join(
        ", "
      )}.`
    )
  }
...
async function initDevelopmentServer(
  ...
  const { develop } = await import("@acmekit/admin-bundler")
...
async function serveProductionBuild(app: Express, options: InitializedOptions) {
  const { serve } = await import("@acmekit/admin-bundler")
