...
- [#7076](https://github.com/medusajs/medusa/pull/7076) [`c3260a2c5a`](https://github.com/medusajs/medusa/commit/c3260a2c5add86ada641db91e834d9f9de62ed14) Thanks [@kasperkristensen](https://github.com/kasperkristensen)! - feat(ui): Re-work `<Toaster />` and `<Toast />` based on `sonner`.
  ...
  This update contains breaking changes to how toasts work in `/ui`. This update has been made to provide a better user experience and to make it easier to use toasts in your Medusa application.

  ### BREAKING CHANGES

  The `useToast` hook has been removed. Users should instead use the `toast` function that is exported from the `/ui` package. This function can be used to show toasts in your application. For more information on how to use the `toast` function, please refer to the documentation.
...
- 8d31ce6: Release of the Medusa UI design system, includes three new packages: `/ui` a set of React components, hooks, and utils; `/icons` a set of React icons; `/ui-preset` a Tailwind CSS preset containing Medusa UI design tokens.
