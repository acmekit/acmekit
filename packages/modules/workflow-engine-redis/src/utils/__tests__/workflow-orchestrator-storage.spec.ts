...
const workflowExecutionServiceMock = {
  ...
  upsert: jest.fn(),
  delete: jest.fn(),
} as unknown as ModulesSdkTypes.IAcmeKitInternalService<any>
...
const baseModuleDeps = {
  ...
  redisQueueName: "acmekit-workflows",
  redisJobQueueName: "acmekit-workflows-jobs",
  logger: loggerMock,
  isWorkerMode: true,
}
...
describe("RedisDistributedTransactionStorage", () => {
  ...
  describe("constructor - Queue configuration", () => {
    it("should create queues with default empty options when no options provided", () => {
      ...
      expect(Queue).toHaveBeenCalledWith("acmekit-workflows", {
        connection: redisMock,
      })

      expect(Queue).toHaveBeenCalledWith("acmekit-workflows-jobs", {
        connection: redisMock,
      })
    ...
    it("should create each queue with different options", () => {
      ...
      expect(Queue).toHaveBeenNthCalledWith(1, "acmekit-workflows", {
        ...mainQueueOptions,
        connection: redisMock,
      })

      expect(Queue).toHaveBeenNthCalledWith(2, "acmekit-workflows-jobs", {
        ...jobQueueOptions,
        connection: redisMock,
      })
