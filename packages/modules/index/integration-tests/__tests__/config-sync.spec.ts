...
import { dbName } from "../__fixtures__/acmekit-config"
...
let acmekitAppLoader!: AcmeKitAppLoader
...
const beforeAll_ = async () => {
  try {
    ...
    acmekitAppLoader = new AcmeKitAppLoader()
    ...
    await acmekitAppLoader.runModulesMigrations()
    const linkPlanner = await acmekitAppLoader.getLinksExecutionPlanner()
    ...
    // Bootstrap modules
    const globalApp = await acmekitAppLoader.load()
    container.register({
...
describe("IndexModuleService syncIndexConfig", function () {
  let acmekitApp: AcmeKitAppOutput
  let indexMetadataService: ModulesSdkTypes.IAcmeKitInternalService<any>
  let indexSyncService: ModulesSdkTypes.IAcmeKitInternalService<any>
  let dataSynchronizer: ModulesSdkTypes.IAcmeKitInternalService<any>
  let onApplicationPrepareShutdown!: () => Promise<void>
  let onApplicationShutdown!: () => Promise<void>

  beforeAll(async () => {
    acmekitApp = await beforeAll_()
    onApplicationPrepareShutdown = acmekitApp.onApplicationPrepareShutdown
    onApplicationShutdown = acmekitApp.onApplicationShutdown
  })
