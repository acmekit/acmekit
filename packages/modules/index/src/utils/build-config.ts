...
function retrieveEntityPropByType({
  ...
  const retrieveFieldNode = (
    ...
    const allFields = [
      ...(astNode?.fields ?? []),
      ...((node?.extensionASTNodes ?? []) as any[]).flatMap(
        (ext: any) => ext.fields ?? []
      ),
    ]
...
function retrieveModuleAndAlias(entityName, moduleJoinerConfigs) {
  ...
  for (const moduleJoinerConfig of moduleJoinerConfigs) {
    ...
    if (moduleSchema) {
      ...
      if (Object.values(linkableKeys).includes(entityName)) {
        relatedModule = moduleJoinerConfig
      }
    }
  ...
  if (!alias) {
    throw new Error(
      `Index Module error, the module ${relatedModule?.serviceName} does not have any alias for the entity ${entityName}. Please add an alias to the module configuration and the entity it correspond to in the args under the entity property.`
    )
  }
...
function processEntity(
  ...
  const schemaParentEntity = Object.values(entitiesMap).filter((value: any) => {
    ...
    const allFields = [
      ...((value.astNode as GraphQLUtils.ObjectTypeDefinitionNode).fields ?? []),
      ...((value.extensionASTNodes ?? []) as any[]).flatMap(
        (ext: any) => ext.fields ?? []
      ),
    ]

    return allFields.some((field: any) => {
...
function buildSchemaFromFilterableLinks(
  ...
  const allFilterable = moduleJoinerConfigs.flatMap((config) => {
    ...
    if (config.isLink) {
      ...
      for (const relationship of config.relationships) {
        ...
        const fieldAliasEntityTypeMap: Record<
          string,
          Record<string, { entity: string; isList: boolean }>
        > = {}
        const primaryServiceName = config.relationships?.[0]?.serviceName
        for (const extend of config.extends ?? []) {
          ...
          /**
           * Only add entity-typed field aliases for the primary entity's
           * extend (relationships[0]). This ensures that the foreign entity
           * (e.g. Brand) appears as a typed field on the primary entity
           * (e.g. Product), enabling the parent-finding code in processEntity
           * to discover the relationship. We skip the inverse side (foreign
           * entity's extend pointing back to primary) to avoid incorrect
           * circular parent relationships in the schema object representation.
           */
          if (extend.serviceName !== primaryServiceName) {
            continue
          }

          fieldAliasEntityTypeMap[extend.serviceName] ??= {}
          for (const [aliasName, aliasConfig] of Object.entries(
            extend.fieldAlias ?? {}
          )) {
            const pathLastSegment = (aliasConfig as any).path
              ?.split?.(".")
              ?.pop?.()
            const targetRel = config.relationships?.find(
              (r) => r.alias === pathLastSegment
            )
            if (targetRel?.entity) {
              fieldAliasEntityTypeMap[extend.serviceName][aliasName] = {
                entity: targetRel.entity,
                isList: !!(aliasConfig as any).isList,
              }
            }
          }
        }
        ...
        entities.push({
          ...
          schema,
          aliasEntityTypes: fieldAliasEntityTypeMap[serviceName] ?? {},
        })
      }
  ...
  const schema = allFilterable
    .map(({ serviceName, entity, fields, schema, aliasEntityTypes = {} }) => {
      if (!schema) {
        return
      }
      ...
      const fieldDefinitions = fields
        .map((field) => {
          ...
          const aliasEntityType = (aliasEntityTypes as Record<
            string,
            { entity: string; isList: boolean }
          >)[field]
          if (aliasEntityType) {
            type = aliasEntityType.isList
              ? `[${aliasEntityType.entity}]`
              : aliasEntityType.entity
          } else {
            type = getGqlType(entity, field) ?? "String"
          }
