...
moduleIntegrationTestRunner<IAuthModuleService>({
  ...
  moduleOptions: {
    cloud: {
      oauth_authorize_endpoint: "https://acmekit.cloud/oauth/authorize",
      oauth_token_endpoint: "https://acmekit.cloud/oauth/token",
  ...
  testSuite: ({ service }) =>
    describe("AcmeKit Cloud Auth provider", () => {
      afterEach(() => {
        mockCache.clear()
      ...
      describe("validateCallback", () => {
        ...
        it("should validate a valid callback", async () => {
          ...
          expect(mockTokenFetch.mock.calls[0][0]).toBe(
            "https://acmekit.cloud/oauth/token"
          )
...
moduleIntegrationTestRunner<IAuthModuleService>({
  ...
  testSuite: ({ service }) =>
    describe("AcmeKit Cloud Auth provider - when cloud options are not provided", () => {
...
moduleIntegrationTestRunner<IAuthModuleService>({
  ...
  testSuite: ({ service }) =>
    describe("AcmeKit Cloud Auth provider - when cloud auth is disabled", () => {
      it("should not enable AcmeKit Cloud Email provider", async () => {
        const error = await service
          .authenticate("cloud", {
            query: {},
