import { IPaymentModuleService } from "@acmekit/framework/types"
...
moduleIntegrationTestRunner<IPaymentModuleService>({
  ...
  testSuite: ({ service }) => {
    describe("Payment Module Service", () => {
      describe("providers", () => {
        it("should load the system and acmekit payments providers by default", async () => {
          ...
          expect(paymentProviders).toEqual(
            expect.arrayContaining([
              ...
              expect.objectContaining({
                id: "pp_acmekit-payments_default",
              }),
            ])
          )
        })
...
moduleIntegrationTestRunner<IPaymentModuleService>({
  ...
  testSuite: ({ service }) =>
    describe("providers", () => {
      it("should not load the acmekit payments provider if the cloud options are not provided", async () => {
        const paymentProviders = await service.listPaymentProviders()
