...
import { AcmeKitPaymentsOptions } from "../types"
import {
...
import {
  ...
  AcmeKitAccountHolder,
  AcmeKitPayment,
  AcmeKitPaymentMethod,
  AcmeKitPaymentMethodSession,
  AcmeKitRefund,
  RefundPaymentRequest,
  UpdateAccountHolderRequest,
} from "../types/acmekit-payments"
...
export class AcmeKitPaymentsProvider extends AbstractPaymentProvider<AcmeKitPaymentsOptions> {
  static identifier = "acmekit-payments"
  protected readonly options_: AcmeKitPaymentsOptions
  ...
  static validateOptions(options: AcmeKitPaymentsOptions): void {
    return validateOptions(options)
  }

  constructor(cradle: Record<string, unknown>, options: AcmeKitPaymentsOptions) {
  ...
  handleStripeError(error: CloudServiceError): HandledErrorType {
    let acmekitPayment: AcmeKitPayment | undefined

    switch (error.type) {
      case "AcmeKitCardError":
        // AcmeKit has created a payment but it failed
        ...
        acmekitPayment = error.data as AcmeKitPayment | undefined
        if (acmekitPayment) {
          return {
            retry: false,
            data: acmekitPayment,
      ...
      case "AcmeKitPaymentUnexpectedStateError":
        acmekitPayment = error.data as AcmeKitPayment | undefined
        if (acmekitPayment) {
          return {
            retry: false,
  ...
  async getPaymentStatus(
    ...
    const statusResponse = this.getStatus(
      payment.data as unknown as AcmeKitPayment
    )
  ...
  async updatePayment({
    ...
    if (isPresent(amount) && data?.amount === amountNumeric) {
      return this.getStatus(
        data as unknown as AcmeKitPayment
  ...
  async savePaymentMethod({
    ...
    })) as AcmeKitPaymentMethodSession
  ...
  private getStatus(payment: AcmeKitPayment) {
  ...
  /**
   * Constructs AcmeKit Payments Webhook event
   * @param {object} data - the data of the webhook request: req.body
   *    ensures integrity of the webhook event
   * @return {object} AcmeKit Payments Webhook event
   */
  constructWebhookEvent(data: ProviderWebhookPayload["payload"]) {
    const signature = data.headers["acmekit-payments-signature"] as string
...
const validateOptions = (options: AcmeKitPaymentsOptions): void => {
  if (!isDefined(options.endpoint)) {
    throw new Error(
      "Required option `endpoint` is missing in AcmeKit payments plugin"
    )
  }
  if (!isDefined(options.webhook_secret)) {
    throw new Error(
      "Required option `webhook_secret` is missing in AcmeKit payments plugin"
    )
  }
  if (!isDefined(options.api_key)) {
    throw new Error(
      "Required option `api_key` is missing in AcmeKit payments plugin"
    )
  }
  ...
  if (
    ...
    throw new Error(
      "Required option `environment_handle` or `sandbox_handle` is missing in AcmeKit payments plugin"
    )
  }
}
