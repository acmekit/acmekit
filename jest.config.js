...
const reAcmeKit = /acmekit$/
const acmekitDir = pkgs.find((p) => reAcmeKit.exec(p))
const acmekitBuildDirs = [`dist`].map((dir) => path.join(acmekitDir, dir))
...
const ignoreDirs = [].concat(
  ...
  builtTestsDirs,
  distDirs,
  "<rootDir>/packages/acmekit-react/*"
)
