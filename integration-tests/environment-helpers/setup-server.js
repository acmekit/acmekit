...
module.exports = async ({ cwd, redisUrl, uploadDir, verbose, env }) => {
  ...
  return await new Promise((resolve, reject) => {
    const acmekitProcess = spawn("node", [path.resolve(serverPath)], {
      cwd,
      env: {
        ...process.env,
    ...
    acmekitProcess.on("error", (err) => {
      console.log(err)
      reject(err)
      process.exit()
    })

    acmekitProcess.on("uncaughtException", (err) => {
      console.log(err)
      reject(err)
      acmekitProcess.kill()
    })

    acmekitProcess.on("message", (port) => {
      setPort(port)
      resolve(acmekitProcess)
    })

    acmekitProcess.on("exit", () => {
