...
import CustomerModule from "@acmekit/acmekit/customer"
import RegionModule from "@acmekit/acmekit/region"
  ...
  testSuite: ({ dbConnection, getContainer, api }) => {
    ...
    describe("Query", () => {
      ...
      it("should handle multiple fieldAlias when multiple links between two modules are defined", async () => {
        ...
        const modulesLoaded = AcmeKitModule.getLoadedModules().map(
        ...
        const linkDefinition = AcmeKitModule.getCustomLinks().map(
          (linkDefinition: any) => {
