...
- `STORE_CORS`: The URLs of storefronts that can access your Medusa backend's `/store` API routes. This is a comma-separated list of URLs that is typically used as the value of the [http.appCors](!docs!/learn/configurations/medusa-config#httpappCors) configuration.
