...
const featureLinks: Record<string, string> = {
  ...
  "auto configuration:":
    "/cloud/projects#prerequisite-acmekit-application-configurations",
  ...
  return (
    <div className="w-full flex flex-col rounded shadow-elevation-card-rest dark:shadow-elevation-card-rest-dark overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center justify-start p-0.5 sm:px-1.5 sm:py-1 border-solid border-r border-acmekit-border-base">
          <p className="txt-medium sm:txt-large text-acmekit-fg-subtle">
            Features
          </p>
        </div>
        ...
        {columns.map((column, index) => (
          <div
            key={index}
            className={clsx(
              "flex items-center justify-center p-0.25 sm:p-1 bg-acmekit-bg-base",
              index !== columns.length - 1 &&
                "border-solid border-r border-acmekit-border-base"
            )}
          >
            <p className="txt-medium sm:txt-large text-acmekit-fg-base text-left w-full">
              {column}
            </p>
          </div>
        ))}
      ...
      {featureSections.map((section) => (
        <div key={section._key} className="w-full">
          {/* Section Header */}
          <div className="w-full p-1.5 bg-acmekit-bg-component flex gap-1 border-acmekit-border-base border-y items-center">
            {featureIcons[section.header.subtitle] && (
              <BorderedIcon
                IconComponent={featureIcons[section.header.subtitle]}
                wrapperClassName="p-[7.5px] bg-acmekit-bg-component rounded-[5px]"
              />
            )}
            <div>
              <H3
              ...
              <P className="text-acmekit-fg-subtle">{section.header.title}</P>
            </div>
          </div>
          ...
          <div className="w-full">
            {section.rows.map((row, index) => (
              <React.Fragment key={row._key}>
                <div
                  className={clsx(
                    "w-full grid gap-0 border-solid border-acmekit-border-base",
                ...
                >
                  {/* Feature name column */}
                  <div className="p-0.25 sm:p-1 flex items-center justify-start border-solid border-r border-acmekit-border-base">
                    <p className="txt-medium-plus text-acmekit-fg-base">
                      <MarkdownContent
                  ...
                  {Array.from({ length: columnCount }, (_, colIndex) => {
                    ...
                    return (
                      ...
                      >
                        <p className="txt-medium text-acmekit-fg-base text-left w-full">
                          <MarkdownContent
                            allowedElements={["br", "a"]}
                            unwrapDisallowed
                          >
