...
import { CheckCircleSolid } from "@acmekit/icons"
            ...
            <React.Fragment key={option._key}>
              <div
                className={clsx(
                  ...
                  index === data.options.length - 1 &&
                    "bg-acmekit-bg-subtle rounded-tr"
                )}
              >
                <div className="flex flex-col w-full">
                  <div className="flex flex-col items-start justify-start w-full">
                    ...
                    <div className="flex flex-col justify-end w-full">
                      <p className="txt-xlarge text-acmekit-fg-subtle">
                        {option.subtitle}
                      </p>
                    </div>
                  </div>
      ...
      >
        ...
        {data.options.map((option, index) => (
          <div
            ...
            className={clsx(
              `p-1 flex items-start border-b border-acmekit-border-base`,
              index !== data.options.length - 1 &&
                "border-solid border-r border-acmekit-border-base",
              index === data.options.length - 1 && "bg-acmekit-bg-subtle"
            )}
          >
            <div className="w-full flex items-start">
              {option.description && (
                <p className="text-acmekit-fg-base txt-small-plus text-balance">
                  {option.description}
                </p>
              )}
            </div>
          ...
          >
            {option.pre_features && (
              <div className="flex items-center w-full">
                <span className="text-acmekit-fg-base txt-small-plus">
                  {option.pre_features}
                </span>
              </div>
            )}
            ...
            {option.features.map((feature, featureIndex) => (
              <div
                key={featureIndex}
                className="flex gap-0.75 items-center w-full text-acmekit-fg-base"
              >
          ...
          <div
            ...
            className={clsx(
              ...
              index !== data.options.length - 1 &&
                "border-solid border-r border-acmekit-border-base",
              index === data.options.length - 1 &&
                "bg-acmekit-bg-subtle rounded-br"
            )}
          >
                ...
                <Link
                  ...
                  href={
                    button.link.path.startsWith("https://")
                      ? button.link.path
                      : `https://acmekit.com${button.link.path}`
                  }
                  target="_blank"
                  rel="noreferrer"
                >
