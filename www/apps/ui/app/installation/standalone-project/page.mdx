...
```bash npm2yarn
npm install /ui-preset --save-dev
```
...
If your project is in a monorepo, you'll need to resolve the path to the `/ui` package from the monorepo root:
...
Medusa's design system packages, including `/ui` and `/ui-preset`, are versioned independently from other `/*` packages. However, they're still released as part of Medusa's releases.
