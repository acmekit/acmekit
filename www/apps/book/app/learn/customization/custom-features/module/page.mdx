...
export const serviceHighlights = [
  ["4", "AcmeKitService", "A service factory that generates data-management methods."]
]
...
The `BrandModuleService` extends a class returned by `AcmeKitService` from the Modules SDK. This function generates a class with data-management methods for your module's data models.

The `AcmeKitService` function receives an object of the module's data models as a parameter, and generates methods to manage those data models. So, the `BrandModuleService` now has methods like `createBrands` and `retrieveBrand` to manage the `Brand` data model.
