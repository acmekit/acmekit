...
- You need to install the following Medusa packages (All these packages use the same version):
    ...
    - `/framework`
    - `/medusa`
    - `/test-utils` (as a dev dependency)
...
import { loadEnv, defineConfig } from "/framework/utils"
...
The Medusa CLI for v2 is now in the `/cli` package. However, you don't need to install it globally. You can just use `npx medusa`, `yarn medusa`, or `pnpm medusa` in your Medusa projects.
...
class BrandModuleService extends AcmeKitService({
  Brand,
}) {
...
The service has automatically generated data-management methods by extending `AcmeKitService` from the Modules SDK. So, you now have methods like `retrieveBrand` and `createBrands` available in the service.
...
In Medusa v1, you imported all Workflows SDK functions and types from the `/workflows-sdk` package.

In Medusa v2, you import them from the `/framework/workflows-sdk` package. For example:
...
In Medusa v1, you imported API-route related types and functions from the `/medusa` package.

In Medusa v2, you import them from the `/framework/http` package. For example:
...
```ts title="src/api/middlewares.ts"
import { defineMiddlewares } from "/framework/http"
...
In Medusa v1, you imported subscriber types from the `/medusa` package.

In Medusa v2, you import them from the `/framework` package. For example:
...
export default async function syncProductsJob(container: AcmeKitContainer) {
