...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        URLs of storefronts that can access the Medusa backend's Store APIs. Only used if [projectConfig.http.appCors](../../configurations/medusa-config/page.mdx#http-appCors-1-1) isn't set in `medusa-config.ts`.
      </Table.Cell>
      <Table.Cell>
        `http://localhost:8000`
      </Table.Cell>
