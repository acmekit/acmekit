...
Consider a Brand Module with a `Brand` data model as shown [in the previous section](#what-is-a-json-property). The [module's service](../../modules/page.mdx#2-create-service) will extend `AcmeKitService`, which generates methods like [updateBrands](!resources!/service-factory-reference/methods/update) to update a brand.
