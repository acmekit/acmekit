...
The `appCors` and `adminCors` properties of Medusa's `http` configuration set the allowed origins for routes starting with `/store` and `/admin` respectively.
        ...
        (
          ...
          return cors({
            origin: parseCorsOrigins(
              configModule.projectConfig.http.appCors
            ),
            credentials: true,
          })(req, res, next)
        },
...
This retrieves the configurations exported from `medusa-config.ts` and applies the `appCors` to routes starting with `/custom`.
