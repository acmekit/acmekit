...
The `AcmeKitRequest` object has a `params` property. `params` holds the path parameters in key-value pairs.
...
You can access all query parameters in the `query` property of the `AcmeKitRequest` object. `query` is an object of key-value pairs, where the key is a query parameter's name, and the value is its value.
...
The Medusa application parses the body of any request with JSON, URL-encoded, or text content types. The request body parameters are set in the `AcmeKitRequest`'s `body` property.
...
The `AcmeKitRequest` type accepts a type argument that indicates the type of the request body. This is useful for auto-completion and to avoid typing errors.
