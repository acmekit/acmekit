...
import { applyLocale, defineMiddlewares } from "/framework/http"
...
After applying the `applyLocale` middleware, you can access the request's locale from the `locale` property of the `AcmeKitRequest` object.
