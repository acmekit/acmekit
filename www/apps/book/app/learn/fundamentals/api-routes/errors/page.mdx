...
When throwing an error in your API routes, middlewares, workflows, or any customization, throw a `AcmeKitError` from the Medusa Framework.
...
The `AcmeKitError` class accepts two parameters in its constructor:

1. The first is the error's type. `AcmeKitError` has a static property `Types` that you can use. `Types` is an enum whose possible values are explained in the next section.
...
- `code`: A common snake-case code. Its values can be:
    ...
    - For other error types, this property won't be available unless you provide a code as a third parameter to the `AcmeKitError` constructor.
...
1. The error thrown. Its type can be `AcmeKitError` or any other error type.
2. A request object of type `AcmeKitRequest`.
