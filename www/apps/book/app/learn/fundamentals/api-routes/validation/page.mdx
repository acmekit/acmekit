...
To use this schema for validating the body parameters of requests to `/custom`, use the `validateAndTransformBody` middleware provided by `/framework/http`. It accepts the Zod schema as a parameter.
...
If a request's body parameters are validated successfully, the middleware sets the validated body parameters in the `validatedBody` property of `AcmeKitRequest`.

### Step 3: Use Validated Body in API Route

In your API route, consume the validated body using the `validatedBody` property of `AcmeKitRequest`.

For example, create the file `src/api/custom/route.ts` with the following content:

export const routeHighlights = [
  ["5", "PostStoreCustomSchemaType", "Infer the request body type from the schema to pass it as a type parameter to `AcmeKitRequest`."],
...
export const POST = async (
  req: AcmeKitRequest<PostStoreCustomSchemaType>,
  res: MedusaResponse
) => {
  res.json({
...
In the API route, you use the `validatedBody` property of `AcmeKitRequest` to access the values of the `a` and `b` properties.

<Note title="Tip">

To pass the request body's type as a type parameter to `AcmeKitRequest`, use Zod's `infer` type that accepts the type of a schema as a parameter.
...
If a request's query parameters are validated successfully, the middleware sets the validated query parameters in the `validatedQuery` property of `AcmeKitRequest`.

### Step 3: Use Validated Query in API Route

Finally, use the validated query in the API route. The `AcmeKitRequest` parameter has a `validatedQuery` parameter that you can use to access the validated parameters.
...
In the API route, you use the `validatedQuery` property of `AcmeKitRequest` to access the values of the `a` and `b` properties as numbers, then return in the response their sum.
