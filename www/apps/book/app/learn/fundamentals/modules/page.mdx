...
export const highlights = [
  ["4", "AcmeKitService", "The service factory function."],
...
Your module's service extends a class generated by `AcmeKitService` from the Modules SDK. This class comes with generated methods for data-management Create, Read, Update, and Delete (CRUD) operations on each of your modules, saving you time that can be spent on building custom business logic.

The `AcmeKitService` function accepts an object of data models to generate methods for. You can pass all data models in your module in this object.
...
Find all methods generated by the `AcmeKitService` in [this reference](!resources!/service-factory-reference)

</Note>

If a module doesn't have data models, such as when it's integrating a third-party service, it doesn't need to extend `AcmeKitService`.
...
As of [Medusa v2.11.0](https://github.com/medusajs/medusa/releases/tag/v2.11.0), MikroORM dependencies are included in the `/framework` package. If you're using an older version of Medusa, change the import statement to `@mikro-orm/migrations`.
...
The workflow has a single step `createPostStep` that creates a post. In the step, you resolve the Blog Module's service from the Medusa container, which the step receives as a parameter. Then, you create the post using the method `createPosts` of the service, which was generated by `AcmeKitService`.
