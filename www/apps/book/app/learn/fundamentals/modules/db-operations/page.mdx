...
2. Add as a last parameter an optional `sharedContext` parameter that has the `AcmeKitContext` decorator from the Modules SDK. This context holds database-related context, including the manager injected by `InjectManager`

For example, in your service, add the following methods:

<Note>

As of [Medusa v2.11.0](https://github.com/medusajs/medusa/releases/tag/v2.11.0), MikroORM dependencies are included in the `/framework` package. If you're using an older version of Medusa, change the import statement to `@mikro-orm/knex`.
...
You add two methods `getCount` and `getCountSql` that have the `InjectManager` decorator. Each of the methods also accept the `sharedContext` parameter which has the `AcmeKitContext` decorator.

The entity manager is injected to the `sharedContext.manager` property, which is an instance of [EntityManager from the `/framework/mikro-orm/knex` package](https://mikro-orm.io/api/knex/class/EntityManager).
...
Both methods must accept as a last parameter an optional `sharedContext` parameter that has the `AcmeKitContext` decorator from the Modules SDK. It holds database-related contexts passed through the Medusa application.
...
export const opHighlights = [
  ...
  ["17", "AcmeKitContext", "A decorator to use Medusa's shared context."],
...
import { InferTypeOf, DAL } from "/framework/types"
...
import { AcmeKitService } from "/framework/utils"
import Post from "./models/post"
import { DAL } from "/framework/types"
...
import { Context } from "/framework/types"
import { EntityManager } from "/framework/mikro-orm/knex"
import { IsolationLevel } from "/framework/mikro-orm/core"
