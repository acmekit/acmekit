...
For example, if you have the following step in your workflow that throws a `AcmeKitError`:
...
const step1 = createStep("step-1", () => {
  throw new AcmeKitError(AcmeKitError.Types.NOT_FOUND, "Item doesn't exist")
})
```
...
If you threw a `AcmeKitError`, then you can check the error message in `errors[0].error.message`.
