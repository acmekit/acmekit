...
This documentation provides a reference to the `createPromotionRulesWorkflow`. It belongs to the `/medusa/core-flows` package.
...
import { createPromotionRulesWorkflow } from "/medusa/core-flows"
...
export async function POST(
  ...
  const { result } = await createPromotionRulesWorkflow(req.scope)
    .run({
      input: {
        // import { RuleType } from "/framework/utils"
