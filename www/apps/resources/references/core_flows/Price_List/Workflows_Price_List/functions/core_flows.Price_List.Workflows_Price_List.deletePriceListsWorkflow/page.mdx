---
slug: /references/medusa-workflows/deletePriceListsWorkflow
sidebar_label: deletePriceListsWorkflow
sidebar_description: Delete one or more price lists.
tags:
  - pricing
  - link
  - workflow
---

import { TypeList, WorkflowDiagram } from "docs-ui"

# deletePriceListsWorkflow - Medusa Core Workflows Reference

This documentation provides a reference to the `deletePriceListsWorkflow`. It belongs to the `/medusa/core-flows` package.

This workflow deletes one or more price lists. It's used by the
[Delete Price List Admin API Route](https://docs.medusajs.com/api/admin#price-lists\_deletepricelistsid).

You can use this workflow within your customizations or your own custom workflows, allowing you to
delete price lists in your custom flows.

<SourceCodeLink link="https://github.com/medusajs/medusa/blob/ad4437b298d499d1a51e54decad6de3f5ebd2181/packages/core/core-flows/src/price-list/workflows/delete-price-lists.ts#L36" />

## Examples

<CodeTabs group="workflow-exection">
    <CodeTab label="API Route" value="api-route">
    
```ts title="src/api/workflow/route.ts"
import type {
  AcmeKitRequest,
  MedusaResponse,
} from "/framework/http"
import { deletePriceListsWorkflow } from "/medusa/core-flows"

export async function POST(
  req: AcmeKitRequest,
  res: MedusaResponse
) {
  const { result } = await deletePriceListsWorkflow(req.scope)
    .run({
      input: {
        ids: ["plist_123"]
      }
    })

  res.send(result)
}
```

    </CodeTab>
    <CodeTab label="Subscriber" value="subscriber">
    
```ts title="src/subscribers/order-placed.ts"
import {
  type SubscriberConfig,
  type SubscriberArgs,
} from "/framework"
import { deletePriceListsWorkflow } from "/medusa/core-flows"

export default async function handleOrderPlaced({
  event: { data },
  container,
}: SubscriberArgs < { id: string } > ) {
  const { result } = await deletePriceListsWorkflow(container)
    .run({
      input: {
        ids: ["plist_123"]
      }
    })

  console.log(result)
}

export const config: SubscriberConfig = {
  event: "order.placed",
}
```

    </CodeTab>
    <CodeTab label="Scheduled Job" value="scheduled-job">
    
```ts title="src/jobs/message-daily.ts"
import { AcmeKitContainer } from "/framework/types"
import { deletePriceListsWorkflow } from "/medusa/core-flows"

export default async function myCustomJob(
  container: AcmeKitContainer
) {
  const { result } = await deletePriceListsWorkflow(container)
    .run({
      input: {
        ids: ["plist_123"]
      }
    })

  console.log(result)
}

export const config = {
  name: "run-once-a-day",
  schedule: "0 0 * * *",
}
```

    </CodeTab>
    <CodeTab label="Another Workflow" value="another-workflow">
    
```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "/framework/workflows-sdk"
import { deletePriceListsWorkflow } from "/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const result = deletePriceListsWorkflow
      .runAsStep({
        input: {
          ids: ["plist_123"]
        }
      })
  }
)
```

    </CodeTab>
  </CodeTabs>

## Steps

<WorkflowDiagram workflow={{"name":"deletePriceListsWorkflow","steps":[{"type":"step","name":"deletePriceListsStep","description":"This step deletes one or more price lists.","link":"../../../Steps_Price_List/functions/core_flows.Price_List.Steps_Price_List.deletePriceListsStep/page.mdx","depth":1},{"type":"step","name":"removeRemoteLinkStep","description":"This step deletes linked records of a record if cascade deletion is enabled.\n\nLearn more in the [Link documentation](https://docs.medusajs.com/learn/fundamentals/module-links/link#cascade-delete-linked-records)","link":"../../../../Common/Steps_Common/functions/core_flows.Common.Steps_Common.removeRemoteLinkStep/page.mdx","depth":2}]}} />

## Input

<TypeList types={[{"name":"DeletePriceListsWorkflowInput","type":"[DeletePriceListsWorkflowInput](../../../../types/core_flows.DeletePriceListsWorkflowInput/page.mdx)","optional":false,"defaultValue":"","description":"The data to delete price lists.","expandable":false,"children":[{"name":"ids","type":"`string`[]","description":"The IDs of the price lists to delete.","optional":false,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.medusajs.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="deletePriceListsWorkflow"/>
