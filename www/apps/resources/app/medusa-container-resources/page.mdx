...
You can use the `ContainerRegistrationKeys` enum imported from `/framework/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      - For Medusa's Commerce and Infrastructural Modules, use the `Modules` enum imported from `/framework/utils`.
...
Use the `ContainerRegistrationKeys` and `Modules` enums imported from `/framework/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      The service of every data model passed to the `AcmeKitService` function that your module's service extends. The service has the methods described in the [service factory reference](../service-factory-reference/page.mdx).
