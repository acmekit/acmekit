...
} from "/medusa/dist/api/admin/api-keys" // <-- Not allowed
```

## Importing from Exposed Directories

The `/medusa` package allows you to import from the following sub-directories:
...
import { 
  AdminGetApiKeysParams,
} from "/medusa/api/admin/api-keys" // <-- Allowed
```
