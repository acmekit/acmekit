...
For Tanstack Query, the error occurs because the package is required by `/dashboard`, but `pnpm` does not automatically install it as a dependency of that package.
...
```bash
pnpm add /dashboard /icons /types /draft-order /ui
```

Make sure to also update these packages whenever you update your Medusa application.

For the Tanstack Query error, you must also install it manually. It must be the same version installed in [/dashboard](https://github.com/medusajs/medusa/blob/d58462c9c91a3e335d03f758438e109caad1f839/packages/admin/dashboard/package.json#L54):
