...
This section of the documentation provides a reference to the methods generated for services extending the service factory (`AcmeKitService`) and how to use them.
...
1. Generate [data-management methods in your main service](#main-service-methods) for each of the data models passed to the `AcmeKitService` function.
2. Generate [internal services](#internal-generated-service-methods) for each of the data models passed to the `AcmeKitService` function, which can be resolved in loaders.
...
- `{DataModelName}` is the pascal-case version of the data model's key that's passed in the object parameter of `AcmeKitService`. The name is pluralized for all operations except the `retrieve` operation.
