...
If the subscription is successful, the method returns an object with the `id` of the subscribed email. If it fails, it throws a `AcmeKitError` with the error message from Mailchimp.
...
You also throw a `AcmeKitError` if the method fails at any point, providing the error message from Mailchimp.
...
- If the template is not recognized, you throw a `AcmeKitError` indicating that the template is invalid.
...
export async function POST(
  req: AcmeKitRequest<z.infer<typeof newsletterSignupSchema>>,
  res: MedusaResponse
) {
