...
// other imports...
import { ProductOptionDTO } from "/framework/types"
...
// other imports...
import { ProductDTO } from "/framework/types"
...
Medusa provides the `useQueryGraphStep` in its `/medusa/core-flows` package. So, you only need to implement the second step.
...
import { ArrowRightMini } from "/icons"
import { clx } from "/ui"
...
You only need to implement the first step, as Medusa provides the other workflows (running as steps) in the `/medusa/core-flows` package.
