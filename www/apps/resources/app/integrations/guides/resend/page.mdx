...
In the `validateOptions` method, you throw an error if the `api_key` or `from` options aren't passed to the module. To throw errors, you use `AcmeKitError` from the Modules SDK. This ensures errors follow Medusa's conventions and are displayed similar to Medusa's errors.
...
import { BigNumberValue, CustomerDTO, OrderDTO } from "/framework/types"
...
import { CreateNotificationDTO } from "/framework/types"
