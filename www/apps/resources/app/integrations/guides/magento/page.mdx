...
export default class MagentoModuleService {
  ...
  async authenticate() {
    ...
    if (!response.ok) {
      throw new AcmeKitError(AcmeKitError.Types.UNAUTHORIZED, `Failed to authenticate with Magento: ${token}`)
    }
...
If the request is successful, you store the access token and its expiration date in the `accessToken` property. If the request fails, you throw a `AcmeKitError` with the error message returned by Magento.
...
You only need to implement the `getMagentoProductsStep` step, which retrieves the products from Magento. The other steps and workflows are provided by Medusa's `/medusa/core-flows` package.
...
You'll now create the workflow that migrates products from Magento using the step you created and steps from Medusa's `/medusa/core-flows` package.
...
You use the `createProductsWorkflow` and `updateProductsWorkflow` workflows from Medusa's `/medusa/core-flows` package to create and update the products in the Medusa application.
