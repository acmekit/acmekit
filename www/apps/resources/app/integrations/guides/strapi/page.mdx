...
This method takes an image ID as a parameter and deletes the corresponding image from Strapi using the Strapi client's files API. If the deletion fails, it throws a `AcmeKitError` with a formatted error message.
...
import { ProductOptionValueDTO } from "/framework/types"
...
import { acquireLockStep, releaseLockStep, useQueryGraphStep } from "/medusa/core-flows"
...
import { createStep, StepResponse } from "/framework/workflows-sdk"
import { Modules } from "/framework/utils"
import { IProductModuleService } from "/framework/types"
...
import { simpleHash, Modules } from "/framework/utils"
import { 
