...
Medusa provides the `useQueryGraphStep` in its `/medusa/core-flows` package. So, you only need to implement the other steps.
...
import { Container, Heading, Button, toast } from "/ui"
...
export async function POST(
  req: AcmeKitRequest<SearchRequest>,
  res: MedusaResponse
) {
