...
The Medusa application’s API routes are guarded by a CORS middleware. Make sure to set the `appCors` property of the `http` configuration in `medusa-config.ts` to the storefront’s URL.
...
module.exports = defineConfig({
  projectConfig: {
    http: {
      appCors: "http://localhost:3000,http://localhost:8000",
      // ...
    },
  },
  // ...
...
Refer to the [Medusa configuration](!docs!/learn/configurations/medusa-config#httpappCors) guide to learn more about configuring CORS in the Medusa application.