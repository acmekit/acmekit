...
class ProductReviewModuleService extends AcmeKitService({
  Review,
}) {
}
...
The `ProductReviewModuleService` extends `AcmeKitService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
The `useQueryGraphStep` step is provided by Medusa in its `/medusa/core-flows` package. So, you only need to implement the `createReviewStep` step.
...
Then, you create the review using the `createReviews` method. As you remember, the Review Module's service extends the `AcmeKitService` which generates data-management methods for you.
...
export async function POST(
  req: AcmeKitRequest<z.infer<typeof PostAdminUpdateReviewsStatusSchema>>, 
  res: MedusaResponse
) {
...
```ts title="src/modules/product-review/service.ts"
import { InjectManager, AcmeKitService, AcmeKitContext } from "/framework/utils"
import Review from "./models/review"
import { Context } from "/framework/types"
import { EntityManager } from "/framework/mikro-orm/knex"
...
class ProductReviewModuleService extends AcmeKitService({
  ...
  async getAverageRating(
    productId: string,
    @AcmeKitContext() sharedContext?: Context<EntityManager>
  ): Promise<number> { 
...
- Pass as the last parameter a context parameter that has the `AcmeKitContext` decorator.
...
import { Button, Input, Label, Textarea, toast, Toaster } from "/ui"
