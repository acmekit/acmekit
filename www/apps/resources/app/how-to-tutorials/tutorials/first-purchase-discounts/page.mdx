...
Medusa provides all these steps in its `/medusa/core-flows` package, so you can implement the workflow right away.
...
import { createWorkflow, when, WorkflowResponse } from "/framework/workflows-sdk"
import { updateCartPromotionsStep, useQueryGraphStep } from "/medusa/core-flows"
...
If any of these validations fail, you throw a `AcmeKitError` with the appropriate error message. This will prevent the promotion from being applied to the cart.
...
If any of these validations fail, you throw a `AcmeKitError` with the appropriate error message. This will prevent the order from being placed if the first-purchase promotion is used by a customer who is not a first-time customer.
