...
import { Github, PlaySolid } from "/icons"
...
class QuoteModuleService extends AcmeKitService({ 
  Quote, 
}) {}
...
The `QuoteModuleService` extends `AcmeKitService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.

So, the `QuoteModuleService` class now has methods like `createQuotes` and `retrieveQuote`.

<Note>

Find all methods generated by the `AcmeKitService` in [this reference](../../../service-factory-reference/page.mdx).
...
import CartModule from "/medusa/cart"
...
The first four steps are provided by Medusa in its `/medusa/core-flows` package. So, you only need to implement the `createQuotesStep` step.
...
Then, you create the quotes using the `createQuotes` method. As you remember, the Quote Module's service extends the `AcmeKitService` which generates data-management methods for you.
...
import { OrderStatus } from "/framework/utils"
import {
  createWorkflow,
  transform,
  WorkflowResponse,
} from "/workflows-sdk"
import { CreateOrderLineItemDTO } from "/framework/types"
...
Then, you create the draft order using the `createOrderWorkflow` workflow which you imported from `/medusa/core-flows`. The workflow creates and returns the created order.
...
```ts title="src/api/store/validators.ts"
// other imports...
import { createFindParams } from "/medusa/api/utils/validators"
...
import { validateAndTransformQuery } from "/framework/http"
...
import {
  createFindParams,
} from "/medusa/api/utils/validators"
...
You create an instance of the JS SDK using the `Medusa` class from the `/js-sdk` package. You pass it an object having the following properties:
...
import { ClientHeaders, FetchError } from "/js-sdk"
import {
  QuoteQueryParams,
  AdminQuotesResponse,
} from "../types"
...
import { AdminOrder } from "/framework/types"
import { Text } from "/ui"
...
As mentioned before, the `useQueryGraphStep` is provided by Medusa's `/medusa/core-flows` package. So, you'll only implement the remaining steps.
...
import { useQueryGraphStep } from "/core-flows"
import { createWorkflow } from "/workflows-sdk"
import { QuoteStatus } from "../modules/quote/models/quote"
...
```tsx
import { AdminOrder, AdminOrderPreview } from "/framework/types"
import {
  Badge,
  CurrencyInput,
  Hint,
...
import { AdminOrder } from "/framework/types"
import { Button, Heading, toast } from "/ui"
...
import { Container, Heading, Toaster } from "/ui"
