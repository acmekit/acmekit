...
If the response contains an error, you throw a `AcmeKitError` with the error message. Otherwise, you return the response, which is of type `PriceResponse`.
...
```ts title="src/workflows/add-custom-to-cart.ts"
import { transform } from "/framework/workflows-sdk"
import { 
  acquireLockStep, 
  addToCartWorkflow, 
...
```ts title="src/workflows/add-custom-to-cart.ts"
import { WorkflowResponse } from "/framework/workflows-sdk"
import { 
  releaseLockStep,
} from "/medusa/core-flows"
```
...
export const POST = async (
  req: AcmeKitRequest<HttpTypes.StoreAddCartLineItem>, 
  res: MedusaResponse
) => {
  const { id } = req.params
...
} from "/medusa/api/store/carts/validators"
