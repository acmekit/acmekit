...
export const POST = async (
  req: AcmeKitRequest<HelloWorldReq>,
  res: MedusaResponse
) => {
  res.json({
...
```ts
import { createProductsWorkflow } from "/medusa/core-flows"
...
To throw errors in an API route, use `AcmeKitError` from the Medusa Framework:

```ts highlights={[["9", "AcmeKitError"]]}
import { AcmeKitRequest, MedusaResponse } from "/framework/http"
...
import { parseCorsOrigins } from "/framework/utils"
...
export default class BlogModuleService extends AcmeKitService({
  Post,
}){
...
The service factory `AcmeKitService` generates data-management methods for your data models.

To extend the service factory in your module's service:

```ts highlights={[["4", "AcmeKitService"]]}
import { AcmeKitService } from "/framework/utils"
import Post from "./models/post"
...
export async function GET(req: AcmeKitRequest, res: MedusaResponse) {
...
import { SubscriberArgs, SubscriberConfig } from "/framework"
import { Modules } from "/framework/utils"
...
Medusa provides a `/test-utils` package with utility tools to create integration tests for your custom API routes, modules, or other Medusa customizations.
