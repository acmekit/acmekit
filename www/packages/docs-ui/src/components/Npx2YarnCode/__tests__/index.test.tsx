...
describe("render", () => {
  test("renders npm2yarn code", () => {
    const { container } = render(
      <Npx2YarnCode npxCode="npx acmekit db:migrate" />
    )
    ...
    expect(npxCodeBlock).toHaveTextContent("npx acmekit db:migrate")
  })

  test("renders npm2yarn code with custom code options", () => {
    const { container } = render(
      <Npx2YarnCode
        npxCode="npx acmekit db:migrate"
        title="Custom Title"
      />
    )
    expect(container).toBeInTheDocument()
  ...
  test("calls npxToYarn with isExecutable=false by default", () => {
    ...
    render(<Npx2YarnCode npxCode="npx acmekit db:migrate" />)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx acmekit db:migrate", "yarn", false)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx acmekit db:migrate", "pnpm", false)
  })

  test("calls npxToYarn with isExecutable=true when specified", () => {
    npxToYarnMock.mockClear()
    render(<Npx2YarnCode npxCode="npx create-acmekit-app@latest" isExecutable={true} />)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx create-acmekit-app@latest", "yarn", true)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx create-acmekit-app@latest", "pnpm", true)
  })

  test("calls npxToYarn with isExecutable=false when explicitly set", () => {
    npxToYarnMock.mockClear()
    render(<Npx2YarnCode npxCode="npx acmekit db:migrate" isExecutable={false} />)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx acmekit db:migrate", "yarn", false)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx acmekit db:migrate", "pnpm", false)
  })
})
