...
describe("rendering", () => {
  ...
  test("applies active styles to selected index", () => {
    ...
    expect(buttons[0]).toHaveClass(
      "rounded-docs_sm text-acmekit-fg-base bg-acmekit-bg-base"
    )
    expect(buttons[0]).toHaveClass(
  ...
  test("applies inactive styles to non-selected indices", () => {
    const { container } = render(<SearchFilters />)
    const buttons = container.querySelectorAll("button")
    expect(buttons[1]).toHaveClass("text-acmekit-fg-muted")
    expect(buttons[1]).not.toHaveClass("bg-acmekit-bg-base")
  })
})
...
describe("interactions", () => {
  ...
  test("updates active state when selectedIndex changes", () => {
    ...
    const { container } = render(<SearchFilters />)
    const buttons = container.querySelectorAll("button")
    expect(buttons[1]).toHaveClass("bg-acmekit-bg-base")
    expect(buttons[0]).toHaveClass("text-acmekit-fg-muted")
  })
})
