...
describe("SearchHits", () => {
  ...
  test("renders community badge for non-AcmeKit integrations", () => {
  ...
  test("does not render badge for AcmeKit integrations", () => {
    const hitWithAcmeKit: HitType = {
      ...mockHits[0],
      integration_vendor: "AcmeKit",
    }
    mockUseHits.mockReturnValue({
      items: [hitWithAcmeKit],
      sendEvent: mockSendEvent,
    })
    const { container } = render(
      <SearchHits indexName="docs" setNoResults={vi.fn()} />
