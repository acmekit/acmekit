 ...
 * 
 * 
 *   Instead, This API route emits the `auth.password_reset` event, passing it the token as a payload. You can listen to that event in a subscriber as explained in [this guide](https://docs.acmekit.com/resources/commerce-modules/auth/reset-password), then send the user a notification. The notification is sent using a [Notification Module Provider](https://docs.acmekit.com/resources/infrastructure-modules/notification), and it should have the URL to reset the password in the AcmeKit Admin dashboard, such as `http://localhost:9000/app/reset-password?token=123`.
 * 
 * 
 *    Use the generated token to update the user's password using the [Reset Password API route](https://docs.acmekit.com/api/admin#auth_postactor_typeauth_providerupdate).
 * externalDocs:
 *   url: https://docs.acmekit.com/resources/commerce-modules/auth/authentication-route#generate-reset-password-token-route
 ...
 *             example: "admin@acmekit-test.com"
 *           metadata:
 *             type: object
 ...
 *       --data-raw '{
 *         "identifier": "admin@acmekit-test.com"
 *       }'
 * tags:
 *   - Auth
 * responses:
