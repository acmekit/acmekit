...
import { AcmeKitEvent } from "types"
...
class OasKindGenerator extends FunctionKindGenerator {
  ...
  readonly RESPONSE_TYPE_NAMES = ["AcmeKitResponse"]
  ...
  getAssociatedWorkflow(node: FunctionNode): string | undefined {
    ...
    // retrieve workflows imported from `@acmekit/core-flows`
    ...
    ;(sourceFile.imports as ts.Node[]).forEach((importNode) => {
      if (
        ...
        importNode.getText() !== `"@acmekit/core-flows"`
      ) {
        return
      }
    ...
    // if no imports found from `@acmekit/core-flows`, return
