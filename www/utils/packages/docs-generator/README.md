...
- Generates JSON documentation files for models built with AcmeKit's DML in `generated/dml-output`.
...
Files under the `packages/acmekit/src/api` or `packages/modules/**/src/models` directories are considered incompatible, so any files under these directories won't have TSDoc docblocks generated for them.
