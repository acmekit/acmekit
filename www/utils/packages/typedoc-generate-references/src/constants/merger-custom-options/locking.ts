...
If you need to refer to an existing implementation as an example, check the [Redis Locking Module Provider in the AcmeKit repository](https://github.com/acmekit/acmekit/tree/develop/packages/modules/providers/locking-redis).`,
      `## 1. Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-locking\`.
...
Create the file \`src/modules/my-locking/service.ts\` that holds the module provider's main service. It must implement the \`ILockingProvider\` interface imported from \`@acmekit/framework/types\`:
...
To use your Locking Module Provider, add it to the \`providers\` array of the Locking Module in \`acmekit-config.ts\`:
...
When you start the AcmeKit application, if your Locking Module Provider is the only registered provider without enabling \`is_default\`, you'll see the following message:
...
In your workflow's step, you can resolve the Locking Module's service from the AcmeKit container:
