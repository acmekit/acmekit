...
If you need to refer to an existing implementation as an example, check the [Google Auth Module Provider in the AcmeKit repository](https://github.com/acmekit/acmekit/tree/develop/packages/modules/providers/auth-google).`,
      `## 1. Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-auth\`.
...
Create the file \`src/modules/my-auth/service.ts\` that holds the module provider's main service. It must extend the \`AbstractAuthModuleProvider\` class imported from \`@acmekit/framework/utils\`:
...
To use your Auth Module Provider, add it to the \`providers\` array of the Auth Module in \`acmekit-config.ts\`:

\`\`\`ts title="acmekit-config.ts"
import { Modules, ContainerRegistrationKeys } from "@acmekit/framework/utils"
...
To test out your Authentication Module Provider, use any of the [Authentication Routes](https://docs.acmekit.com/resources/commerce-modules/auth/authentication-route), using your provider's ID as a path parameter.
...
You can then use the token to create a customer by sending a request to the [Create Customer API route](https://docs.acmekit.com/api/store#customers_postcustomers).
      `,
    ],
  },
}
