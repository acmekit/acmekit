...
The Caching Module and its providers are available starting [AcmeKit v2.11.0](https://github.com/acmekit/acmekit/releases/tag/v2.11.0).
...
If you need to refer to an existing implementation as an example, check the [Redis Caching Module Provider in the AcmeKit repository](https://github.com/acmekit/acmekit/tree/develop/packages/modules/providers/caching-redis).`,
      `## 1. Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-caching\`.
...
Create the file \`src/modules/my-caching/service.ts\` that holds the module provider's main service. It must implement the \`ICachingProviderService\` interface imported from \`@acmekit/framework/types\`:
...
To use your Caching Module Provider, add it to the \`providers\` array of the Caching Module in \`acmekit-config.ts\`:
...
Then, start your AcmeKit server with the following command:
...
In your workflow's step, you can resolve the Caching Module's service from the AcmeKit container:
