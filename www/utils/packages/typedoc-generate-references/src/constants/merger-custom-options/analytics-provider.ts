...
If you need to refer to an existing implementation as an example, check the [PostHog Analytics Module Provider in the AcmeKit repository](https://github.com/acmekit/acmekit/tree/develop/packages/modules/providers/analytics-posthog).`,
      `## Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-analytics\`.
...
Create the file \`src/modules/my-analytics/service.ts\` that holds the implementation of the module provider's main service. It must extend the \`AbstractAnalyticsProviderService\` class imported from \`@acmekit/framework/utils\`:
...
To use your Analytics Module Provider, add it to the \`providers\` array of the Analytics Module in \`acmekit-config.ts\`:
...
module.exports = defineConfig({
  ...
  modules: [
    {
      resolve: "@acmekit/acmekit/analytics",
      options: {
        providers: [
          {
...
In the step, you resolve the service of the Analytics Module from the [AcmeKit container](!docs!/learn/fundamentals/acmekit-container) and use its \`track\` method to track the event. This method will use the underlying provider configured (which is your provider, in this case) to track the event.
...
You'll now track the order creation event whenever an order is placed in your AcmeKit application. You can test this out by placing an order and checking in your third-party provider if the event was tracked successfully.
`,
