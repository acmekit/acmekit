...
If you need to refer to an existing implementation as an example, check the [S3 File Module Provider in the AcmeKit repository](https://github.com/acmekit/acmekit/tree/develop/packages/modules/providers/file-s3).`,
      `## Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-file\`.
...
Create the file \`src/modules/my-file/service.ts\` that holds the implementation of the module provider's main service. It must extend the \`AbstractFileProviderService\` class imported from \`@acmekit/framework/utils\`:
...
To use your File Module Provider, add it to the \`providers\` array of the File Module in \`acmekit-config.ts\`:
...
To test out your File Module Provider, use the AcmeKit Admin or the [Upload API route](https://docs.acmekit.com/v2/api/admin#uploads_postuploads) to upload a file.
`,
    ],
  },
}
