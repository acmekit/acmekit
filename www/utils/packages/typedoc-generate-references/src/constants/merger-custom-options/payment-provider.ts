...
If you need to refer to an existing implementation as an example, check the [Stripe Payment Module Provider in the AcmeKit repository](https://github.com/acmekit/acmekit/tree/develop/packages/modules/providers/payment-stripe).`,
      `## Understanding Payment Module Provider Implementation

The Payment Module Provider handles processing payment with a third-party provider. However, it's not responsible for managing payment concepts within AcmeKit, such as payment sessions or collections. These concepts are handled by the Payment Module which uses your Payment Module Provider within core operations.
...
If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-payment\`.
...
Create the file \`src/modules/my-payment/service.ts\` that holds the module provider's main service. It must extend the \`AbstractPaymentProvider\` class imported from \`@acmekit/framework/utils\`:
...
To use your Payment Module Provider, add it to the \`providers\` array of the Payment Module in \`acmekit-config.ts\`:
...
Before you use your Payment Module Provider, enable it in a region using the AcmeKit Admin.

Then, go through checkout to place an order. Your Payment Module Provider is used to authorize the payment.
`,
      `## Useful Guides

- [Storefront Guide: how to implement UI for your Payment Module Provider during checkout](https://docs.acmekit.com/resources/storefront-development/checkout/payment)
`,
    ],
  },
}
