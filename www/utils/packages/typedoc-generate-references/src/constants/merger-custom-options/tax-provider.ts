...
A Tax Module Provider is used to retrieve the tax lines in a provided context. The Tax Module provides a default \`system\` provider. You can create your own Tax Module Provider, either in a plugin, in a module provider, or directly in your AcmeKit application's codebase, then use it in any tax region.`,
      `## Implementation Example

As you implement your Tax Module Provider, it can be useful to refer to an existing provider and how it's implemeted.

If you need to refer to an existing implementation as an example, check the [Avalara Tax Module Provider tutorial](https://docs.acmekit.com/resources/integrations/guides/avalara).`,
      `## Understanding Tax Module Provider Implementation

The Tax Module Provider handles calculating taxes with a third-party provider. However, it's not responsible for managing tax concepts within AcmeKit, such as creating a tax region. The Tax Module uses your Tax Module Provider within core operations.
...
If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-tax\`.
...
Create the file \`src/modules/my-tax/service.ts\` that holds the module provider's main service. It must implement the \`ITaxProvider\` interface imported from \`@acmekit/framework/types\`:
...
To use your Tax Module Provider, add it to the \`providers\` array of the Tax Module in \`acmekit-config.ts\`:
