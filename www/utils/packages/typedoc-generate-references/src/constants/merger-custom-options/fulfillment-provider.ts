...
The Fulfillment Module Provider handles processing fulfillments and shipments with a third-party provider. However, it's not responsible for managing fulfillment concepts within AcmeKit, such as creating a fulfillment or its shipments. The Fulfillment Module uses your Fulfillment Module Provider within core operations.

For example, when the merchant creates a fulfillment for an order, the Fulfillment Module uses your Fulfillment Module Provider to create the fulfillment in the third-party system, then creates the fulfillment in AcmeKit. So, you only have to implement the third-party fulfillment processing logic in your Fulfillment Module Provider.
`,
      `## 1. Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a AcmeKit application, create it under the \`src/modules\` directory. For example, \`src/modules/my-fulfillment\`.
...
Create the file \`src/modules/my-fulfillment/service.ts\` that holds the module provider's main service. It must extend the \`AbstractFulfillmentProviderService\` class imported from \`@acmekit/framework/utils\`:
...
To use your Fulfillment Module Provider, add it to the \`providers\` array of the Fulfillment Module in \`acmekit-config.ts\`:
...
Before you use your Fulfillment Module Provider, in the AcmeKit Admin:
...
Then, place an order, choosing the shipping option you created during checkout, and create a fulfillment in the AcmeKit Admin. The fulfillment is created using your provider.
`,
    ],
  },
}
