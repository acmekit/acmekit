...
const mergerOptions: Partial<TypeDocOptions> = {
  ...
  name: "references",
  indexTitle: "AcmeKit References",
  plugin: [...(baseOptions.plugin || []), "typedoc-plugin-markdown-acmekit"],
  ...
  formatting: {
    "*": {
      ...
      parameterComponentExtraProps: {
        expandUrl:
          "https://docs.acmekit.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation",
      },
    },
    internal: {
      maxLevel: 1,
...
You should only use this methods when implementing complex customizations. For common cases, check out [available workflows instead](/acmekit-workflows-reference).
...
You should only use the methods in this reference when implementing complex customizations. For common cases, check out [available workflows instead](/acmekit-workflows-reference).
