...
export default function (theme: MarkdownTheme) {
  Handlebars.registerHelper(
    ...
    function (this: SignatureReflection): string {
      ...
      let str = `${Handlebars.helpers.titleLevel()} Hooks\n\nHooks allow you to inject custom functionalities into the workflow. You'll receive data from the workflow, as well as additional data sent through an HTTP request.\n\nLearn more about [Hooks](https://docs.acmekit.com/learn/fundamentals/workflows/workflow-hooks) and [Additional Data](https://docs.acmekit.com/learn/fundamentals/api-routes/additional-data).\n\n`
      ...
      hooks.forEach(({ hook, isNested }) => {
        ...
        if (isNested) {
          str += `\n\n:::note\n\nThis hook is nested within a [when](https://docs.acmekit.com/learn/fundamentals/workflows/conditions) condition, so it may not be executed if the when condition isn't satisfied.\n\n:::\n\n`
        }
